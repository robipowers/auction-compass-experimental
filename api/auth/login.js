// Auction Mentor: Magic Link Login (Dashboard App)
...
const APP_URL = process.env.APP_URL || 'https://app.auctionmentor.io';
...
export default async function handler(req, res) {
    ...
    try {
        ...
        const { error: insertError } = await supabase
            .from('auth_tokens')
            .insert({ token, user_id: user.id, email: cleanEmail, expires_at: expiresAt });
        ...
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; background: #0d120f; color: #e0e0e0; padding: 40px 30px; border-radius: 12px;">
            <div style="text-align: center; margin-bottom: 30px;">
                ...
                <p style="color: #909090; font-size: 13px; margin-top: 8px;">Secure Login Link — Dashboard</p>
            </div>
            <p style="color: #c8c8c8; line-height: 1.7; font-size: 15px;">
                Click the button below to log in to your dashboard. This link expires in <strong style="color: #fff;">${TOKEN_EXPIRY_MINUTES} minutes</strong> and can only be used once.
            </p>
        ...
        await supabase.from('audit_log').insert({
            ...
            details: { email: cleanEmail, app: 'dashboard' },
            performed_by: 'system',
        });
