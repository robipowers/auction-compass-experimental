...
export class MarketDataService {
  ...
  /**
   * Connect to market data (mock for development)
   */
  async connect(): Promise<void> {
    ...
    // Start mock price updates if enabled
  ...
  private startMockUpdates(): void {
    ...
    this.intervalId = setInterval(() => {
      this.config.instruments.forEach(instrument => {
        ...
        const movement = (Math.random() - 0.48) * tickSize * 4; // Slight upward bias
        const newPrice = Math.round((current.price + movement) / tickSize) * tickSize;
        
        // Calculate session change (from "open")
        const basePrice = MOCK_BASE_PRICES[instrument] || current.price;
        const sessionOpen = basePrice - 15 + Math.random() * 30; // Randomize "open"
  ...
  /**
   * Simulate reconnection (for testing disconnection handling)
   */
