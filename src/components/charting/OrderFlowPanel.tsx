...
export function OrderFlowPanel({ data, height = 300, className }: OrderFlowPanelProps) {
  ...
  const imbalances = useMemo(() => {
    return data.footprint.filter((level) => {
      ...
      return ratio > 2.5; // 2.5:1 imbalance threshold
    });
  }, [data]);
      ...
      <CardContent>
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="delta" className="mt-0">
            <div
              className="relative bg-background/50 rounded-lg border border-border p-4"
              style={{ height }}
            >
                  ...
                  return (
                    ...
                    >
                      <div
                        ...
                        style={{ height: "50%", width: 24 }}
                      >
                        {isPositive && (
                          <div
                            className="w-5 bg-gradient-to-t from-green-600 to-green-400 rounded-t-sm"
                      ...
                      >
                        {!isPositive && (
                          <div
                            className="w-5 bg-gradient-to-b from-red-600 to-red-400 rounded-b-sm"
                            style={{ height: `${barHeight}%` }}
                          />
                        )}
                      </div>
          ...
          <TabsContent value="cumulative" className="mt-0">
            ...
            >
              <svg width="100%" height="100%" className="overflow-visible">
                ...
                <polyline
                  fill="none"
                  stroke={data.cumulativeDelta >= 0 ? "#10b981" : "#ef4444"}
                  strokeWidth="2"
                  points={data.deltaBars
                    .reduce<{ x: number; y: number; cumDelta: number }[]>((acc, bar, i) => {
          ...
          <TabsContent value="footprint" className="mt-0">
            <div
              className="bg-background/50 rounded-lg border border-border overflow-auto"
              style={{ height }}
            >
                    ...
                    return (
                      <tr key={i} className="border-b border-border/50">
                        <td className={cn(
                          ...
                          isBidImbalance && "bg-red-500/20 font-bold text-red-400"
                        )}>
                          {level.bid.toLocaleString()}
                        </td>
                        <td className="py-1.5 px-3 text-center text-muted-foreground font-medium">
                          {level.price.toLocaleString()}
                        </td>
                        <td className={cn(
                          "py-1.5 px-3 text-right",
                          isAskImbalance && "bg-green-500/20 font-bold text-green-400"
                        )}>
...
export function generateMockOrderFlow(): OrderFlowData {
  ...
  for (let i = 0; i < 8; i++) {
    footprint.push({
      price: 18455 - i * 5,
      bid: Math.floor(Math.random() * 1200) + 200,
      ask: Math.floor(Math.random() * 1200) + 200,
    });
  }
  ...
  return {
    ...
    sessionDelta: Math.floor(cumDelta * 0.6),
    footprint,
    divergences: cumDelta > 1000 ? [] : [{ type: "bearish", price: 18450, time: "10:30" }],
  };
}
