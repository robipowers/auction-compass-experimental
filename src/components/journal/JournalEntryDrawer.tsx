      ...
      <SheetContent className="w-full sm:max-w-[540px] overflow-y-auto" side="right">
        ...
        <div className="space-y-6 py-6">
            ...
            <div>
              ...
              <div className="flex gap-2 mt-1">
                <Button
                  ...
                  size="sm"
                  className={cn(
                    formData.direction === "long" && "bg-green-500/20 text-green-400 border-green-500/50 hover:bg-green-500/30"
                  )}
                ...
                <Button
                  ...
                  size="sm"
                  className={cn(
                    formData.direction === "short" && "bg-red-500/20 text-red-400 border-red-500/50 hover:bg-red-500/30"
                  )}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => updateField("pnl_pips", e.target.value)}
                />
              </div>
              <div>
                <Label>P&L ($)</Label>
          ...
          <div className="space-y-4">
            ...
            <EmotionSelector
              label="Pre-Trade"
              selected={formData.pre_trade_emotion}
              onChange={(emotions) => updateField("pre_trade_emotion", emotions)}
            />
            ...
            {!isQuickEntry && (
              <>
                <EmotionSelector
                  label="During Trade"
                  selected={formData.during_trade_emotion}
                  onChange={(emotions) => updateField("during_trade_emotion", emotions)}
                />
                <EmotionSelector
                  label="Post-Trade"
                  selected={formData.post_trade_emotion}
                  onChange={(emotions) => updateField("post_trade_emotion", emotions)}
                />
              </>
            )}
          </div>

          {/* Section: Plan Connection (full entry only) */}
          {!isQuickEntry && (
            <div className="space-y-4">
              <h3 className="text-sm font-semibold uppercase text-muted-foreground tracking-wider">
                Plan Connection
              ...
              <div>
                ...
                <Input
                  placeholder="e.g. Scenario A - Bullish breakout above 18,400"
                  value={formData.scenario_traded}
                  onChange={(e) => updateField("scenario_traded", e.target.value)}
                />
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex gap-2">
                  <Button
                    type="button"
                    variant={formData.followed_plan === true ? "default" : "outline"}
                    size="sm"
                    className={cn(formData.followed_plan === true && "bg-green-500/20 text-green-400")}
                    onClick={() => updateField("followed_plan", formData.followed_plan === true ? null : true)}
                  >
                    Yes
                  </Button>
                  <Button
                    type="button"
                    variant={formData.followed_plan === false ? "default" : "outline"}
                    size="sm"
                    className={cn(formData.followed_plan === false && "bg-red-500/20 text-red-400")}
                    onClick={() => updateField("followed_plan", formData.followed_plan === false ? null : false)}
                  >
                    No
                  </Button>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Select
                    value={formData.scenario_validation_status || "none"}
                    onValueChange={(v) =>
                      updateField("scenario_validation_status", v === "none" ? null : (v as ScenarioValidationStatus))
                    }
                  >
                ...
                <div>
                  ...
                  <Select
                    value={formData.entry_timing || "none"}
                    onValueChange={(v) =>
                      updateField("entry_timing", v === "none" ? null : (v as EntryTiming))
                    }
                  >
                    <SelectTrigger><SelectValue placeholder="Select..." /></SelectTrigger>
                    <SelectContent>
          ...
          {/* Section: Screenshots (full entry only) */}
          {!isQuickEntry && (
            <div className="space-y-4">
          ...
          {/* Section: Reflection (full entry only) */}
          {!isQuickEntry && (
            <div className="space-y-4">
              <h3 className="text-sm font-semibold uppercase text-muted-foreground tracking-wider">
                Reflection
              ...
              <div>
                ...
                <Textarea
                  ...
                  onChange={(e) => updateField("what_to_improve", e.target.value)}
                  rows={2}
                />
              </div>
              ...
              <div>
                ...
                <Textarea
                  placeholder="What's the main takeaway from this trade?"
                  value={formData.lesson_learned}
                  onChange={(e) => updateField("lesson_learned", e.target.value)}
                  rows={2}
                />
              </div>
            </div>
        ...
        <SheetFooter className="pt-4 border-t border-border/50 flex gap-2">
          ...
          <Button onClick={handleSave} className="flex-1">
            <Save className="h-4 w-4 mr-1" /> {editingEntry ? "Update Entry" : "Save Entry"}
          </Button>
        </SheetFooter>
      </SheetContent>
    </Sheet>
